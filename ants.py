        ...
        if self.container:
            if self.ant == None:
                if not other.container:
                    return True
            return False
        if isinstance(other, BodyguardAnt):
            return False
        return True
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while place:
            bees = place.bees
            if not bees:
                if place.entrance == None or place.entrance is hive:
                    return None
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 4A
        p = self.place
        allbees = self.place.bees
        if self.armor - amount <= 0:
            beescopy = list(allbees[::-1])
            for i in beescopy:
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        ...
        if place.bees:
            allbees = place.bees
            beescopy = list(allbees[0:])
            for bee in beescopy:
                bee.armor -= 1
                if bee.armor <= 0:
                    allbees.remove(bee)
                #print(str(beescopy) + str(all))
    ...
    def action(self, colony):
        ...
        if self.impostor:
            ...
            if place.ant is not self:
                antscopy.append(place.ant)
            while place.exit:
                place = place.exit
                if place.ant:
                    if isinstance(place.ant, BodyguardAnt):
                        if place.ant.ant:
                            antscopy.append(place.ant.ant)
                    antscopy.append(place.ant)
            antscopy = [ant for ant in antscopy if ant not in self.doubled_ants]
            for ant in antscopy:
                ant.damage *= 2
            self.doubled_ants += [ant for ant in antscopy if ant not in self.doubled_ants]
